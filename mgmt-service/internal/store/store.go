...
func (s *Store) ListAPIKeys(ctx context.Context, namespace string) ([]core.APIKey, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, name, key_scope, key_mask, created_at, last_used_at
		FROM api_key
		WHERE namespace = ?
...
func (s *Store) CreateAPIKey(
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, ?, ?)`,
			key.ID, namespace, key.Name, key.Scope, key.Mask, key.Digest)
		if err != nil {
...
func scanAPIKey(row scanner) (core.APIKey, error) {
	var key core.APIKey
	if err := row.Scan(
		&key.ID, &key.Name, &key.Scope, &key.Mask, &key.CreatedAt, &key.LastUsedAt,
	); err != nil {
