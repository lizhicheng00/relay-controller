...
func (s *Store) Provision(
	...
	err = tx.QueryRowContext(ctx, `
		SELECT id FROM api_key WHERE namespace = ? AND slot = 0 FOR UPDATE`,
		identity.Namespace).Scan(&existingDefaultID)
	switch {
...
func (s *Store) ListAPIKeys(ctx context.Context, namespace string) ([]core.APIKey, error) {
	rows, err := s.db.QueryContext(ctx, `
		...
		ORDER BY k.slot`, namespace)
	if err != nil {
		return nil, fmt.Errorf("list API keys: %w", err)
	}
	defer rows.Close()

	keys := make([]core.APIKey, 0, core.MaxAdditionalAPIKeys+1)
	for rows.Next() {
