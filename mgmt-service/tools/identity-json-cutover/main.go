...
func convert(namespacesJSON, usersJSON string, output io.Writer, codec *secret.Codec) error {
	...
	for index, item := range identities {
		...
		_, _ = fmt.Fprintf(output, "    ('main', UNHEX('%s'), UNHEX('%s'), '%s')%s\n",
			hex.EncodeToString(accountKey), hex.EncodeToString(memberKey),
...
func joinIdentities(namespacesJSON, usersJSON string) ([]identity, error) {
	...
	for _, user := range users {
		if existing := usersByCustomer[user.CustomerID]; existing != "" && existing != user.UserID {
	...
	for _, namespace := range namespaces {
		userID := usersByCustomer[namespace.DomainID]
		if userID == "" {
			return nil, fmt.Errorf("namespace %q has no matching user", namespace.Namespace)
		}
